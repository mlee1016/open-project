...
korean_study_ist = []
for i in range(len(All_Italian_lists)):
   
  korean_study_ist.append(f'{i+1}: {All_Italian_lists[i].de_ko_name}')
...
if options != None or options == "":
  ...
  studying_list:list = All_Italian_lists[int(str(options[0]))-1].de_ko_List
  ...
  try:  
    ...
    input_r = st.text_input(f"Type the meaning of this phrase '{current_Phrases}' : ",placeholder="Type in English/Italian",autocomplete=None)
    if on:
