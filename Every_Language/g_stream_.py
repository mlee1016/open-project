...
def make_alpha(string:str):
   ...
   #st.rerun()
korean_study_ist = []
for i in range(len(All_German_lists)):
   
  korean_study_ist.append(f'{i+1}: {All_German_lists[i].de_ko_name}')
...
if options != None or options == "":
  ...
  studying_list:list = All_German_lists[int(str(options[0]))-1].de_ko_List
  ...
  on = st.toggle("German -> English (Note : need a German Keyboard)")
  if on:
  ...
  try:  
    ...
    input_r = st.text_input(f"Type the meaning of this phrase '{current_Phrases}' : ",placeholder="Type in English/German",autocomplete=None)
    if on:
