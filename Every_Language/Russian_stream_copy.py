...
if options != None or options == "":
  ...
  on = st.toggle("Russian -> English (Note : need a Russian Keyboard)")
  if on:
      
      studying_list = en_ko
      #for i in range (len(list(en_ko[st.session_state.a][1]))):
      shuffle_list = list(en_ko[st.session_state.a_ru][1])
  ...
  if 'ls' not in st.session_state:
    st.session_state.ls = []
  if 'a_ru' not in st.session_state:
    st.session_state['a_ru'] = 0
  question = get_n()
  #st.write(korean_present_polite.de_ko_List[0][1])
  
  try:  
    if st.session_state.a_ru+1 <= 15:
      st.write(f"{st.session_state.a_ru+1}","/",str(len(studying_list))) 
    ...
    current_Phrases = studying_list[st.session_state.a_ru][0]
    if on3:
      current_Phrases = ""
      
    input_r = st.text_input(f"Type the meaning of this phrase '{current_Phrases}' : ",placeholder="Type in English/Russian",autocomplete=None)
    if on:
      st.write("Shuffled Phrase:")
      st.write(*shuffle_list)
    str1_norm = make_alpha(input_r)
    str2_norm = make_alpha(str(studying_list[st.session_state.a_ru][1]))
    ...
    if str1_norm.upper() == str2_norm.upper():
      ...
      st.session_state.ls.append(f"{st.session_state.a_ru}")
      ...
      st.write(error)
      st.write(f"%{(st.session_state.a-error.__len__())/(st.session_state.a_ru-1)*100}")
